...
from tkinter import filedialog  # 서브모듈은 별도로 선언 (파일 열기)
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일 선택", filetypes=(
        ("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir="C:/")
    # 사용자가 선택한 파일 목록을 리스트 박스에 넣기
    for file in files:
...
# 선택 삭제
def del_file():
    # 선택된 파일의 인덱스 활용, 뒤 인덱스부터 지워야 인덱스 변조 방지 가능
    # .reverse()는 실제로 뒤집어짐, reversed()는 뒤집어진 새로운 객체 생성
...
def merge_image():
    try:
        ...
        # 포맷
        img_format = cmb_format.get().lower()  # PNG, JPG, BMP가져와서 소문자로
        ...
        # size[0] ==> 가로너비 , size[1] ==> 세로높이
        widths, heights = zip(*(image_sizes))  # unzip 활용
        # 최대 넓이, 전체 높이 구하기
        max_width, total_height = max(widths), sum(heights)
        # 스케치북 준비
        if img_space > 0:  # 이미지간격 옵션 적용
